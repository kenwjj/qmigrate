...
Depends on: schema-spec.md v0.1 (the §6 internal representation)

## 1. Purpose

The differ compares a **declared** schema (the §6 internal representation produced by the DSL loader) against the **current** on-disk state of an HDB, and emits a classified set of differences. It only *detects and classifies*; the downstream plan and apply layers act on the result.

The public entry points read the HDB directly. Internally the work is split so that the actual file I/O (introspection) is separate from the comparison logic, which is pure and depends only on two §6 representations.
...
|------|------|-----------|----------------|
| `.qm.i.introspect` | yes | `[hdbPath; table]` → §6 rep, or `(::)` if absent | Read one on-disk table into a §6 representation |
| `.qm.i.compare` | no (pure) | `[declared; actual; opts]` → diff rows | Compare two §6 reps, classify each difference |
...
- **In-memory tables** are never read from disk (see §6).
...
| `enumMismatch` | §9: enumerated vs raw-symbol mismatch | warning |
...
- `enumMismatch`: for partitioned tables a symbol column is enumerated by default (§9); an on-disk symbol column with no enum domain (`f` empty) is a mismatch. Splayed/in-memory symbol columns are raw, so no enumeration is expected there.
...
rows: flip `table`column`change`from`to`severity`detail ! (
  ...
  // change:   symbol — from the §4 catalog
...
- **Divergent partition schemas** → not detected in Phase 1 (latest-partition assumption, §3.1).
...
2. Run `.qm.diff` / `.qm.diffTable` against declared schemas that have been mutated to provoke each row in the §4 catalog (add/drop/type/attr/list/order/kind/partition/enum/unmanaged/new/skipped).
...
- Reading Delta Control XML (the differ consumes the §6 rep regardless of source).
